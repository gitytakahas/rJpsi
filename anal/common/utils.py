...
def getChannel(channel):
    ...
    # b-tag
    if 'bbbb' in channel: text += ", tight b tag"
    elif 'bbb' in channel: text += ", loose b tag"
    elif 'bb' in channel: text += ", tight b tag" if isAH else ", 2 b tag"
    elif 'b' in channel: text += ", loose b tag" if isAH else ", 1 b tag"
    # region
...
#    if not onTop: latex.DrawLatex(0.15, 0.985)
#    else: latex.DrawLatex(0.20, 0.99, "CMS")
    latex.DrawLatex(0.20, 0.95, "CMS")
    latex.SetTextSize(0.04)
...
def drawAnalysis(s, center=False):
    ...
    if "Zh" in s: text = "X' #rightarrow ZH #rightarrow q#bar{q}#tau#tau"
    elif "Wh" in s: text = "X' #rightarrow WH #rightarrow q#bar{q}#tau#tau"
    elif "hh" in s: text = "X  #rightarrow HH #rightarrow b#bar{b}#tau#tau"
    elif "DM" in s or "CR" in s: text = "DM + heavy flavour"
    elif "VH" in s or "Vh" in s or s.startswith('X'): text = "X #rightarrow VH #rightarrow q#bar{q}#tau#tau"
    else: return True
    latex = TLatex()
    latex.SetNDC()
    latex.SetTextSize(0.04)
